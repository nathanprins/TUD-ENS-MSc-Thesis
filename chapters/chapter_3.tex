...
Algorithm \ref{alg:conn_params_control_system} shows the pseudocode of the algorithm. As the name would suggest, \texttt{AdaptConnParamsToVdd} contains the logic for calculating the new connection parameters based on the capacitor voltage. It starts by calculating the change of voltage since the last time the function was called. Since the function is called every time the system sleeps, which is dependent on the connection rate, the rate of change has to be normalised to seconds, which results in $rate_{normalised}$. 

A target rate $rate_{target}$ is selected based on the voltage level of the capacitor. If the voltage is low then a positive charge rate is chosen, if it is low then a negative charge rate is chosen. For anything in between the charge rate is set to zero to try and maintain the voltage.
...
\texttt{LoadPreset} copies the new preset into the advertisement data array. If the preset is different than the one that was already loaded a \textbf{true} is returned, otherwise a \textbf{false} is returned. Afterwards, the network can be dropped using \textbf{Fast Reconnect}, the device will start advertising the new PPCP, and the host will reconnect using these new parameters.
